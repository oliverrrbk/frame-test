// ============================================================================
// SupplierPicker.jsx — søgbar leverandørvælger i Bison Frame-stil.
// Klon af CustomerPicker, tilpasset leverandør-biblioteket. Vælg en gemt
// leverandør (fx Davidsen) i materialeliste-flowet i stedet for at taste forfra.
// Åbner via portal (klippes aldrig i en modal), glas-look, live-søgning.
//
// Brug:
//   <SupplierPicker
//      suppliers={suppliers}          // [{ id, name, contact_name, email, ... }]
//      value={supplierId}             // valgt id (eller null)
//      onSelect={(s) => ...}          // hele leverandør-objektet
//      onClear={() => ...}            // ryd valg (valgfrit)
//      onCreateNew={() => ...}        // "+ Ny leverandør" i bunden (valgfrit)
//   />
// ============================================================================
import React, { useState, useRef, useEffect, useLayoutEffect } from 'react';
import { createPortal } from 'react-dom';
import { ChevronDown, Check, Search, Truck, Plus, X } from 'lucide-react';

export default function SupplierPicker({ suppliers = [], value = null, onSelect, onClear = null, onCreateNew = null, placeholder = 'Vælg leverandør fra biblioteket…', disabled = false }) {
    ...
    return (
        <>
            ...
            >
                {selected ? <Truck size={18} color="#3b82f6" /> : <Search size={18} color="#94a3b8" />}
                ...
                >
                    <div style={{ position: 'relative', padding: '2px 2px 6px' }}>
                        ...
                        <input
                            ...
                            placeholder="Søg navn, kontakt, mail…"
                    ...
                    <div style={{ overflowY: 'auto', flex: 1, WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', touchAction: 'pan-y' }}>
                        ...
                        {filtered.map(s => {
                            ...
                            const sub = [s.contact_name, s.email || s.phone].filter(Boolean).join(' · ');
                            return (
                                <button
                                    key={s.id}
                                    type="button"
                                    onClick={() => { onSelect && onSelect(s); setOpen(false); setQuery(''); }}
                                    style={{
                                ...
                                >
                                    <Truck size={16} style={{ flexShrink: 0, color: active ? '#1d4ed8' : '#64748b' }} />
                                    <span style={{ flex: 1, minWidth: 0 }}>
                                        <span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: active ? '#1d4ed8' : '#334155', fontWeight: active ? 700 : 600, fontSize: '.95rem' }}>{s.name}</span>
                    ...
                    {onCreateNew && (
                        ...
                        >
                            <Plus size={16} /> Opret ny leverandør
                        </button>
                    )}
