...
| `supabase/setup_suppliers.sql` | **Leverandør-bibliotek.** Ny tabel `suppliers` (ét leverandørkort pr. FIRMA, `carpenter_id` = firma-rod: navn/kontaktperson/mail/telefon/adresse/zip/by/notes/logo_url + `updated_at`-trigger) + firma-afgrænset RLS (SELECT/INSERT/UPDATE/DELETE, samme mønster som `customers`). INGEN backfill. Idempotent. Bruges af "Leverandører"-fanen (under Kunder, `SupplierLibrary.jsx`) + leverandørvælger (`SupplierPicker.jsx`) i `MaterialListBuilder.jsx`. |
