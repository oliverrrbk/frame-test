...
export default function AdminTimesheet({ leadsData, profile }) {
    ...
    const filteredEntries = useMemo(() => {
        return allEntries.filter(entry => {
            const matchMonth = entry.date.startsWith(selectedMonth);
            const matchUser = selectedUser === 'all' || entry.employeeId === selectedUser;
            return matchMonth && matchUser;
        });
    }, [allEntries, selectedMonth, selectedUser]);
    ...
    const handleExportCSV = () => {
        ...
        link.setAttribute("download", `Loen_Timer_${selectedMonth}.csv`);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 0.3s ease-out', maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    <div style={{ position: 'relative' }}>
                        <select 
                            ...
                            style={{ padding: '12px 40px 12px 16px', borderRadius: '12px', border: '1px solid rgba(0,0,0,0.1)', backgroundColor: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(8px)', outline: 'none', cursor: 'pointer', appearance: 'none', fontWeight: '600', color: '#1e293b', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}
                        >
                            <option value="all">Alle medarbejdere</option>
                            {teamMembers.map(m => (
                                <option key={m.id} value={m.id}>{m.owner_name || m.company_name} ({m.role})</option>
                            ))}
                        </select>
                    ...
                    <input 
                        ...
                        style={{ padding: '12px 16px', borderRadius: '12px', border: '1px solid rgba(0,0,0,0.1)', backgroundColor: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(8px)', outline: 'none', cursor: 'pointer', fontWeight: '600', color: '#1e293b', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}
                    />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
                <div className="glass-panel" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <span style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Samlede Timer</span>
            ...
            <div className="glass-panel" style={{ overflow: 'hidden', padding: 0 }}>
                {filteredEntries.length === 0 ? (
                    <div style={{ padding: '64px', textAlign: 'center', color: '#94a3b8' }}>
                        ...
                        <p style={{ margin: 0, fontSize: '1rem' }}>Prøv at ændre måned eller medarbejder.</p>
                    </div>
                ) : (
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '950px' }}>
                            ...
                            <tbody>
                                {filteredEntries.map((entry, idx) => {
                                    ...
                                    return (
                                        <tr key={entry.id} style={{ borderBottom: idx === filteredEntries.length - 1 ? 'none' : '1px solid rgba(0,0,0,0.04)', transition: 'background 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'rgba(0,0,0,0.02)'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
            ...
            {showModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100000, padding: '20px', animation: 'fadeIn 0.2s ease-out' }}>
                    <div style={{ width: '100%', maxWidth: '600px', background: '#fff', borderRadius: '16px', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
                        ...
                        <form onSubmit={handleSaveEntry} style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    ...
                                    <select 
                                        ...
                                        style={{ padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none', fontSize: '0.95rem' }}
                                    >
                                        <option value="">-- Vælg medarbejder --</option>
                                        {teamMembers.map(m => (
                                            <option key={m.id} value={m.id}>{m.owner_name || m.company_name}</option>
                                        ))}
                                    </select>
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                ...
                                >
                                    ...
                                    <optgroup label="Aktive Byggeprojekter">
                                        {leadsData.filter(l => ['Sendt tilbud', 'Bekræftet opgave', 'Historik'].includes(l.status)).map(l => (
                                            <option key={l.id} value={l.id}>Sag {l.case_number || String(l.id).substring(0,6)} - {l.customer_name}</option>
                                        ))}
                                    </optgroup>
                                    <optgroup label="Internt">
                                        <option value="internal">Fravær, Sygdom, Ferie, Møder o.lign.</option>
                                    </optgroup>
                                </select>
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    ...
                                    <input 
                                        ...
                                        style={{ padding: '12px', borderRadius: '8px', border: '2px solid #3b82f6', outline: 'none', fontSize: '0.95rem', fontWeight: 'bold', backgroundColor: '#eff6ff' }}
                                    />
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                ...
                                <textarea 
                                    ...
                                    style={{ padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none', fontSize: '0.95rem', fontFamily: 'inherit', resize: 'vertical' }}
                                />
                            </div>
